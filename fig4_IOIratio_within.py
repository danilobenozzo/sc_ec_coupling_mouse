...
def plot_ratio_within(pwd_data):
    ...
    for i_label, label_i in enumerate(label_path):
        ...
        for i_plot, n_sim_i in enumerate(n_sim):
            ...
            dataset = pickle.load(open('%s%s/%s/output_onlyRate_within_%s.pickle' % (pwd_data, n_sim_i, label_i, n_sim_i)))
        ...
        pwd_tosave = pwd_data
        filename = 'in_out_onlyRatio_within_perNetwork_bar_iso_%s' % label_figure[i_label]
